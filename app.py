...
def ai():
    ...
    def send_message_to_genai(query):
        return chat_model.send_message(query).text
    if request.method == 'POST':
        ...
        with ThreadPoolExecutor() as executor:
            future = executor.submit(send_message_to_genai, query)
            try:
                gemini_response = future.result(timeout=10)
            except FuturesTimeoutError:
                return jsonify("Request timed out. Please try again later.")
            except Exception:
